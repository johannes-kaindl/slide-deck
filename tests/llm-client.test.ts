...
describe("DeckLlmClient.generate", () => {
  it("returns streamed content on success", async () => {
    const c = new DeckLlmClient({ url: "http://x" }, "m", fakeHttp(() => ({ status: 200 })), okStream({ content: "# Title", finishReason: "stop" }));
  ...
  it("throws the envelope message on a 200-error stream body", async () => {
    ...
    const c = new DeckLlmClient({ url: "http://x" }, "m", fakeHttp(() => ({ status: 200 })), errStream);
  ...
  it("falls back to non-streaming on StreamNetworkError (CORS)", async () => {
    ...
    const c = new DeckLlmClient({ url: "http://x" }, "m", fakeHttp(() => ({ status: 200, json: { choices: [{ message: { content: "# Fallback" }, finish_reason: "stop" }] }, text: '{"choices":[]}' })), netErr);
  ...
  it("does NOT fall back on AbortError", async () => {
    ...
    const c = new DeckLlmClient({ url: "http://x" }, "m", fakeHttp(() => ({ status: 200 })), abortErr);
  ...
  it("fallback surfaces the envelope from the error body", async () => {
    const netErr: any = () => { const e = new Error("net"); e.name = "StreamNetworkError"; return Promise.reject(e); };
    const c = new DeckLlmClient({ url: "http://x" }, "m", fakeHttp(() => ({ status: 200, json: {}, text: '{"error":{"message":"context length exceeded"}}' })), netErr);
  ...
  it("skips streaming on the second call after a CORS fallback (C6)", async () => {
    ...
    const c = new DeckLlmClient({ url: "http://x" }, "m", fakeHttp(() => ({ status: 200, json: { choices: [{ message: { content: "# A" } }] }, text: "{}" })), netErr);
  ...
  it("never sends suppress params to an always-on thinker", async () => {
    ...
    const c = new DeckLlmClient({ url: "http://x:1" }, "gpt-oss-20b", http, (() => { const e = new Error("x"); e.name = "StreamNetworkError"; throw e; }) as never);
...
describe("probe", () => {
  ...
  it("does not throw when the injected http fn throws synchronously", async () => {
    ...
    const c = new DeckLlmClient({ url: "http://x:1" }, "m", syncThrow, (() => { throw new Error("nope"); }) as never);
