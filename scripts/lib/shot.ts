...
export async function setWindowSize(cdp: Cdp, width: number, height: number): Promise<boolean> {
  return Boolean(await cdp.evaluate<boolean>(`
    ...
    // ABWEICHUNG vom Original (3d-codeblocks): erst den macOS-Vollbildmodus aufheben.
    // Im Vollbild nimmt setSize() die Vorgabe entgegen und tut nichts — getSize() meldet
    // danach die Bildschirmgroesse. Der Treiber glaubt dann, er habe die Masse gesetzt,
    // und nimmt in Wahrheit in einer geraetetabhaengigen Groesse auf (gemessen
    // 2026-08-16: 1500x940 angefordert, 1512x949 bekommen). Reproduzierbarkeit ueber
    // Geraete ist der ganze Grund fuer eine feste Groesse.
    if (win.isFullScreen()) {
    ...
    // Ehrlich melden statt still danebenliegen: ein zu kleines Display kann die Vorgabe
    // kappen, und dann stimmen die Ausschnitte im Rezept nicht mehr.
    if (Math.abs(w - ${width}) > 8 || Math.abs(h - ${height}) > 8) {
      console.warn("Fenstergroesse abgelehnt: " + ${width} + "x" + ${height} + " angefordert, " + w + "x" + h + " bekommen");
    }
    return true;
  `));
}
