...
export function buildVault(spec: VaultSpec): string[] {
  ...
  // ABWEICHUNG vom Original (3d-codeblocks): dort ist der Generator-Aufruf Pflicht. Er ist
  // aber pluginspezifisch (dort: 3D-Modelle) und steht damit quer zu dem, was der Kopf
  // dieser Datei ueber sich behauptet — "die pluginneutrale Haelfte". Hier optional: ein
  // Fixture ohne erzeugte Beilagen ist der Normalfall, nicht der Sonderfall.
  const generator = join(fixtureDir, "make-fixture.mjs");
  if (existsSync(generator)) {
    const erzeugt = execFileSync("node", [generator, vaultDir], { encoding: "utf-8" });
    log.push("Beilagen erzeugt:" + erzeugt.replace(/\n$/, "").replace(/\n/g, "\n   "));
  }
